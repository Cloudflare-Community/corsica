...
export const getArtifact: Handler<SimpleAuthedEnvironment> = async (
	ctx,
	next,
) => {
	...
	if (globalThis.MINIFLARE) {
		console.log(
			`Received GET request for "${req.param("hash")}" from user "${get(
				"ASSOCIATED_USER",
			)}" with User-Agent "${req.headers.get("user-agent") || ""}".`,
		);
	} else {
