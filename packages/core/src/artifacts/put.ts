...
export const putArtifact: Handler<SimpleAuthedEnvironment> = async ({
	req,
	env,
	get,
}) => {
	...
	let expirationDuration = parseInt(
		req.headers.get("x-artifact-duration") || "",
	);
	...
	await Promise.all([
		env.R2.put(objectName, req.body, {
			httpMetadata: {
				contentType:
		...
		env.EXPIRE.get(env.EXPIRE.idFromName(objectName)).fetch(
			"https://turbo.internal/" + objectName,
			{
				headers: {
	...
	if (globalThis.MINIFLARE) {
		console.log(
			`Received PUT request for artifact "${req.param(
				"hash",
			)}" from user "${get("ASSOCIATED_USER")}" with User-Agent "${
				req.headers.get("user-agent") || ""
			}" which expires at ${new Date(expireAt).toLocaleDateString("en-UK")}.`,
		);
	} else {
		env.AE.writeDataPoint({
			blobs: [
				...
				get("ASSOCIATED_USER"),
				req.headers.get("user-agent") || "",
				req.headers.get("x-artifact-tag") || "",
			],
			doubles: [expireAt],
		});
	}
