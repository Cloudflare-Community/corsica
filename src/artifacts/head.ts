...
export const headArtifact: Handler<Environment> = async ({ req, env, get, executionCtx }) => {
  ...
  if(globalThis.MINIFLARE) {
    console.log(`Received HEAD request for artifact "${req.param("hash")}" from user "${get("ASSOCIATED_USER")}" with User-Agent "${req.headers.get("user-agent") || ""}".`);
  } else {
    env.AE.writeDataPoint({
      blobs: [
        "HEAD", req.param("hash"), req.headers.get("user-agent") || ""
      ]
    });
  }
